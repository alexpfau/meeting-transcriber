...
/// The sample-rate interrogation ladder for a freshly built aggregate device,
/// split out of `AppAudioCapture.swift` to stay under the 600-line lint cap.
///
/// Pure CoreAudio property reads plus the pure `SampleRateQuery.chooseRate`
/// decision they feed. Nothing here touches instance state, which is why it
/// lifts cleanly: `startCapture` owns the object lifecycle, and a hundred lines
/// of property reads sitting in the middle of it buried the ordering that
/// actually matters. `internal` rather than `private` so that file can still
/// reach them.
