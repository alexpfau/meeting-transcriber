...
/// only after proving the current anchor delivers not one nonzero sample.
///
...
extension AppAudioCapture {
    ...
    func resolveOutputAnchor() -> String? {
        ...
        currentAnchorUID.withLock { $0 = selection.candidate.uid }
    ...
    private func logAnchorSelection(
        ...
        // have gone silent.
        logger.error(
            "App audio: anchoring the tap to candidate \(selection.position, privacy: .public) of \(candidateCount, privacy: .public) (\(reason, privacy: .public)) after the previous anchor delivered only digital silence",
        )
    }
    ...
    func observeForDigitalSilence(zeroSamples: Int, samples: Int) {
        ...
        case let .silenceDetected(mayRestart):
            ...
            DispatchQueue.main.async { [weak self] in
                self?.handleDigitalSilenceDetected()
            }
    ...
    private func handleDigitalSilenceDetected() {
